...
#This is a modified version of https://empslocal.ex.ac.uk/people/staff/gv219/codes/linearshallowwater.py, where we've changed it to a 2.5-layer model with nonlinear continuity. We've added RK4 rather than Euler timestep. Boundary conditions also are changed in places.

import numpy as np
import copy


#use this function to import in everything that we defined in the execute files.
...
#Here we make the mass input terms (m1 for top layer, m2 for bottom layer) optional by setting them as =0 by default (e.g. no mas input  by default). Extent is the number of grid cells that the mass is input over
    ...
    m2=m2o
    extent=extento
    start_t=start_to #this is the pickup time
...
def update_boundaries():
    ...
    #Solid walls except where optional periodic channel; i.e., no zonal (u) flow through the left and right walls unless in channel
    ...
    _v1[0, tau_min:tau_max+2] = _v1[-2, tau_min:tau_max+2]
    ...
    ## OUTSIDE PERIODIC REGION
    #vs on east and west walls are 0 (no slip)
    ...
    #hs on east/west walls are zero-derivative (free slip)
    ...
    #us on east and west walls are 0 if outside periodic channel
    _u1[0, 0:tau_min+1] = 0
    _u1[1, 0:tau_min+1] = 0
    _u1[-1, 0:tau_min+1] = 0
    _u1[-2, 0:tau_min+1] = 0
...
def update_boundaries_intermediate(bdd_u1,bdd_v1,bdd_h1,bdd_u2,bdd_v2,bdd_h2,u1_next, h1_next, u2_next, h2_next): #update boundaries so the intermediate rk steps obey
    ...
    bdd_v1[0, tau_min:tau_max+2] = bdd_v1[-2, tau_min:tau_max+2] 
    ...
    state2=np.array([bdd_u1[1:-1, 1:-1],bdd_v1[1:-1, 1:-1],bdd_h1[1:-1, 1:-1], bdd_u2[1:-1, 1:-1], bdd_v2[1:-1, 1:-1], bdd_h2[1:-1, 1:-1]],dtype=object) 
    for field in state2:
    	# fix corners to be average of neighbours
        field[0, 0] =  0.5*(field[1, 0] + field[0, 1])
        field[-1, 0] = 0.5*(field[-2, 0] + field[-1, 1])
...
def diffx(psi):
    global dx
   # Calculate d/dx over single grid square and return at x points at midpoint between x values of input array
    #i.e. d/dx(psi)[i,j] = (psi[i+1/2, j] - psi[i-1/2, j]) / dx
    return (psi[1:,:] - psi[:-1,:]) / dx

def diff2x(psi):
    #Calculate d^2/dx^2 over a single grid square and return at same x points as those of the input array
...
def diff2y(psi):
   #Calculate d^2/dy^2 over a single grid square and return at same y points as those of the input array
...
def diffy(psi):
   # Calculate d/dy over single grid square and return at y points at midpoint between y values of input array
...
def rhs():
   ...
   # Calculate the right hand side of the equations (u,v,h) in both layers
    ...
    h1_rhs = -h1*divergence1()-x_average(u1)*x_average(dh1dx)-y_average(v1)*y_average(dh1dy) 
    h2_rhs = -h2*divergence2()-x_average(u2)*x_average(dh2dx)-y_average(v2)*y_average(dh2dy)
    
    if extent>0:
        if np.size(m1)==1: #if given one value (not a vector in time)
        ...
        if np.size(m1)>1: #if given a vector, which must be in time
            h1_rhs[:,ny//7:ny//7+extent] += m1[int((t-start_t)/dt)]
        if np.size(m2)>1:
            h2_rhs[:,ny//7:ny//7+extent] += m2[int((t-start_t)/dt)]
    
    #if optionally turn on a mass transfer between layers
    h1_rhs+=vol_transfer 
    h2_rhs+=-vol_transfer
    ...
    #form the next k functions, for rk4
    def k(k_prev,step):
